...
if (Test-Path $SentryDir) {
    Write-Host "⚠️  Sentry directory already exists at $SentryDir" -ForegroundColor Yellow
    ...
    if ($response -eq 'y' -or $response -eq 'Y') {
        ...
        Write-Host "ℹ️  Using existing installation" -ForegroundColor Cyan
...
if ($PortInUse) {
    Write-Host "⚠️  Port 9000 is already in use (probably MinIO)" -ForegroundColor Yellow
...
Write-Host "⚠️  You will be asked to create an admin user" -ForegroundColor Yellow
Write-Host ""

# Check if already installed
if (Test-Path "sentry\sentry.conf.py") {
    Write-Host "ℹ️  Sentry appears to be already installed" -ForegroundColor Cyan
    $response = Read-Host "Skip installation? (Y/n)"
    if ($response -ne 'n' -and $response -ne 'N') {
        Write-Host "⏭️  Skipping installation" -ForegroundColor Cyan
    } else {
        .\install.sh
    }
} else {
    # Run install.sh (requires WSL or Git Bash)
    Write-Host "⚠️  Running install.sh requires WSL or Git Bash" -ForegroundColor Yellow
...
while ($Counter -lt $MaxWait) {
    ...
    if ($Counter % 10 -eq 0) {
        Write-Host "Still waiting... ($Counter seconds)" -ForegroundColor Cyan
    }
...
if (-not $Ready) {
    Write-Host "⚠️  Timeout waiting for Sentry. Check logs with: docker compose logs -f web" -ForegroundColor Yellow
}
