...
Write-Host "🚀 Starting Quado Framework Services" -ForegroundColor Cyan
...
foreach ($item in $portsToCheck) {
    ...
    if ($connection) {
        Write-Host "⚠️  Port $($item.Port) is in use by $($item.Service)" -ForegroundColor Yellow
    } else {
        Write-Host "✅ Port $($item.Port) is free ($($item.Service))" -ForegroundColor Green
    }
}

# Step 3: Start base services (PostgreSQL, Redis, MinIO)
Write-Host ""
Write-Host "📋 Step 3: Starting base services..." -ForegroundColor Yellow
...
foreach ($service in $services) {
    if ($service.Service -in @('postgres', 'redis', 'minio')) {
        $status = if ($service.Health -eq "healthy" -or $service.State -eq "running") { "✅" } else { "⚠️" }
        Write-Host "$status $($service.Service): $($service.State)" -ForegroundColor $(if ($status -eq "✅") { "Green" } else { "Yellow" })
    }
}
...
Write-Host ""
Write-Host "======================================" -ForegroundColor Cyan
Write-Host "📍 Service URLs & Ports" -ForegroundColor Cyan
Write-Host "======================================" -ForegroundColor Cyan
Write-Host ""
...
Write-Host ""
Write-Host "🐛 Error Tracking:" -ForegroundColor Yellow
Write-Host "   Sentry: http://localhost:9000 (ready to install)" -ForegroundColor White
Write-Host ""

# Step 5: Ask about Sentry
Write-Host "======================================" -ForegroundColor Cyan
Write-Host "📋 Step 4: Sentry Installation" -ForegroundColor Yellow
Write-Host "======================================" -ForegroundColor Cyan
Write-Host ""
...
if ($response -eq 'y' -or $response -eq 'Y') {
    Write-Host ""
    Write-Host "🚀 Starting Sentry installation..." -ForegroundColor Cyan
    & "$PSScriptRoot\setup-sentry.ps1"
} else {
    Write-Host ""
    Write-Host "ℹ️  Sentry installation skipped." -ForegroundColor Cyan
