...
foreach ($item in $portsToCheck) {
    ...
    if ($connection) {
        Write-Host "  Port $($item.Port) is in use by $($item.Service)" -ForegroundColor Yellow
    } else {
...
foreach ($service in $services) {
    if ($service.Service -in @('postgres', 'redis', 'minio')) {
        $status = if ($service.Health -eq "healthy" -or $service.State -eq "running") { "OK" } else { "WARN" }
...
{
    Write-Host ""
    Write-Host "Sentry installation skipped." -ForegroundColor Cyan
